    ...
    try {
      ...
      if (cached) {
        ...
        if (Array.isArray(parsed)) {
          // 过滤无效手机号，确保至少有一个输入框
          const validPhones = parsed.filter((p) => !p || isValidPhone(p));
          return validPhones.length > 0 ? validPhones : [''];
        }
      }
    } catch (e) {
      console.error('Failed to load cached phones:', e);
    }
    return [''];
  };

  // 保存手机号到缓存
  const savePhonesToCache = (phones: string[]) => {
    try {
      // 只过滤完全空的输入，保留所有非空值（包括重复的）
      const toSave = phones.filter((p) => p.trim() !== '');
      localStorage.setItem('cached_phones', JSON.stringify(toSave));
    } catch (e) {
      console.error('Failed to save phones to cache:', e);
    }
  };
